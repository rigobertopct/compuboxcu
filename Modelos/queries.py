...
class Query(graphene.ObjectType):
    ...
    def resolve_pugiles(self, info, name):
        if name == "":
            ...
            return Pugil.objects.filter(
                Q(nombre__icontains=name) | Q(edad__icontains=name) | Q(peso__icontains=name) | Q(
                    categoria__categoria__icontains=name) | Q(pais__pais__icontains=name)
    ...
    def resolve_combate(self, info, name):
        if name == "":
            ...
            return Pugil.objects.filter(
                Q(fecha__icontains=name) | Q(esquinaA__nombre__icontains=name) | Q(
                    esquinaR__nombre__icontains=name) | Q(evento__nombre__icontains=name)
